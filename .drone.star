            ...
            if "skip" not in matrix or not matrix["skip"]:
                ...
                for storage in params["storages"]:
                    for run_with_remote_php in params["withRemotePhp"]:
                        ocis_url = OCIS_URL
                        if params["k3d"] and ctx.build.event == "cron":
                        ...
                        pipeline = {
                            ...
                            "steps": skipIfUnchanged(ctx, "acceptance-tests") +
                                     ...
                                     (waitK3sCluster() + deployOcis() + waitForOcis(ocis_url = ocis_url) + ocisServicePods() if params["k3d"] and ctx.build.event == "cron" else ocisServer(storage, extra_server_environment = params["extraServerEnvironment"], with_wrapper = True, tika_enabled = params["tikaNeeded"], volumes = ([stepVolumeOcisStorage]))) +
                            ...
                            "services": (emailService() if params["emailNeeded"] else []) +
                                        ...
                                        (k3sCluster() if params["k3d"] and ctx.build.event == "cron" else []),
